...
2. **Prod exposure — flag is UI-only.** `checkFeatureEnabled()` exists in `src/app/actions/advancement/utils.ts:18` but is **called zero times** by any of the 59 advancement server actions. `ADVANCEMENT_TRACKING=false` gates only page renders (`advancement/page.tsx`, `bulk-entry/page.tsx`). Next.js server actions are live POST endpoints regardless of UI gating — so the CHUCK-27 cross-unit IDOR surface is plausibly reachable in production **today**, not merely "after a flag flip." This raises CHUCK-27's urgency under *every* option, including freeze, unless a server-side kill-switch lands first.
