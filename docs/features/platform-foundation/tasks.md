...
|---|---|---|---|
| PLATFORM-013 | CHUCK-19: install `@sentry/nextjs` — `src/instrumentation.ts` (+`onRequestError`), `src/instrumentation-client.ts`, `sentry.server.config.ts`, `sentry.edge.config.ts`, `withSentryConfig` in `next.config.mjs`, env plumbing (no-op without DSN), `.env.local.example` + `docs/tech.md` updates | 2026-07-11 | (this commit) |
