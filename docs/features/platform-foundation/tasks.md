...
|---|---|---|---|
| PLATFORM-024 | CHUCK-9: `/api/square/payments` — `journal_lines` insert failure now deletes the orphan journal entry and returns 500 with `squarePaymentId` (was: log-and-continue leaving a zero-line entry while money was captured); TDD via new handler-level `tests/unit/api/square-payments-route.test.ts` | 2026-07-12 | (this commit) |
