...
|---|---|---|---|
| PLATFORM-015 | CHUCK-19: extend `src/lib/logger.ts` — `.error()` forwards to Sentry (`captureException` for Error data, `captureMessage` otherwise) with `logger` namespace tag + message/data extras; forwarding unconditional even for disabled loggers; TDD via `tests/unit/lib/logger.test.ts` (7 tests) | 2026-07-11 | (next commit) |
