...
| PLATFORM-013 | CHUCK-19: install `@sentry/nextjs` — `src/instrumentation.ts` (+`onRequestError`), `src/instrumentation-client.ts`, `sentry.server.config.ts`, `sentry.edge.config.ts`, `withSentryConfig` in `next.config.mjs`, env plumbing (no-op without DSN), `.env.local.example` + `docs/tech.md` updates | no | none | Not Started | `make build` green with and without DSN; dev boots |
| PLATFORM-014 | CHUCK-19: wire error boundaries + context — `Sentry.captureException` in `(dashboard)/error.tsx` + `global-error.tsx`, client `Sentry.setUser` from session, release + environment on all inits | no | PLATFORM-013 | Not Started | forced dev exception in Sentry with release + user context |
