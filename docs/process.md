...
- **Linear tool prefix:** the skills call Linear via `mcp__claude_ai_Linear__*` (claude.ai connector). If the connection type changes (e.g. to a local `linear-server` MCP), update the `allowed-tools` frontmatter in `implement`, `qa`, `presentation`, and `epic-progress`.
