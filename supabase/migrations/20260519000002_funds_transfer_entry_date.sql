-- Extend transfer_funds_to_billing RPC to optionally accept an entry_date.
-- Fixes chronological-ordering inconsistency where funds transfers wrote
-- CURRENT_DATE (server-side UTC) while cash payments wrote client-local dates.
-- New callers pass the local date; old callers default to CURRENT_DATE (today's
